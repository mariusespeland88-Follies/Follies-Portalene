...
function pickImageFlexible(a: any): string | null {
  return (
    a?.coverUrl || a?.cover_url || a?.cover ||
    a?.image_url || a?.image || a?.bannerUrl || a?.banner_url ||
    a?.thumb || a?.thumbnail || a?.avatar || a?.logo || a?.icon ||
    a?.media?.cover || a?.media?.image || a?.image?.url || a?.cover?.url ||
    a?.assets?.cover || a?.assets?.image || null
  );
}
function pickAccentFlexible(a: any): string | null {
  const v =
    a?.accent || a?.accentColor || a?.color || a?.themeColor ||
    a?.primary_color || a?.style?.accent || null;
...
export default function ActivityDetailPage() {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      if (!id) { setErr("Mangler aktivitets-ID i URLen."); setLoading(false); return; }
      try {
        ...
        if (!alive) return;
        if (!a) { setErr(`Fant ikke aktiviteten (id: ${id}).`); setLoading(false); return; }
        setAct(a);
        ...
        if (alive) { setErr("Noe gikk galt ved innlasting av aktiviteten."); setLoading(false); }
      }
    })();
    return () => { alive = false; };
  }, [id, supabase]);
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 text-neutral-900">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/90">
                { (act as any).start_date ? `Start: ${(act as any).start_date}` : "Start: —"} · { (act as any).end_date ? `Slutt: ${(act as any).end_date}` : "Slutt: —"}
              </p>
            </div>
          </div>
      ...
      <div className="mt-6 border-b border-neutral-200 flex gap-6">
        {([
          ...
          <button key={key} onClick={() => setTab(key)} className={`pb-2 -mb-px ${tab === key ? "border-b-2 border-red-600 text-neutral-900" : "text-neutral-500 hover:text-neutral-800"}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <section className="lg:col-span-2 space-y-6">
          ...
          {tab === "ledere" && (
            <PeoplePanel title="Ledere" people={leaders} emphasize={true} variant="leaders" busyId={busyId} onDemote={async (mid) => await setRole(mid, "participant")} />
          )}
          ...
          {tab === "okter" && (
            <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              <div className="flex items-center justify-between">
                ...
                <Link href={`/activities/${encodeURIComponent(String(act.id))}/sessions/new`} className="rounded-lg bg-red-600 px-3.5 py-2 text-sm font-semibold text-white hover:bg-red-700">
                  Lag ny økt
                </Link>
              </div>
                  ...
                  {sessions.map((s) => (
                    <li key={s.id} className="py-3 flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="font-medium text-neutral-900 truncate">{s.title}</div>
        ...
        <aside className="space-y-6">
          <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
            ...
            <dl className="mt-3 text-sm text-neutral-700 space-y-2">
              <div className="flex justify-between gap-4"><dt>Type</dt><dd className="font-medium">{labelForType((act as any)?.type)}</dd></div>
...
function PeoplePanel({
  title, people, emphasize, variant, onPromote, onDemote, busyId,
}: {
  title: string;
  people: AnyObj[];
  emphasize?: boolean;
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
      <div className="flex items-center justify-between">
        ...
        <span className="inline-flex items-center rounded-full bg-black/85 px-3 py-1 text-xs font-semibold text-white ring-1 ring-white/10">{people.length}</span>
      </div>
            ...
            return (
              <li key={mid} className="flex items-center justify-between gap-3 py-3">
                <div>
                  ...
                  <p className="text-xs text-neutral-700">{email ? <span>{email}</span> : <span className="text-neutral-500">Ingen e-post</span>}{phone ? <span> · {phone}</span> : null}</p>
                </div>
                <div className="flex items-center gap-2">
                  <Link href={`/members/${mid}`} className="rounded-lg bg-white px-3 py-1.5 text-sm font-semibold text-neutral-900 ring-1 ring-neutral-300 hover:bg-neutral-100">Åpne</Link>
                  <Link href={`/members/${mid}/edit`} className="rounded-lg bg-red-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-red-700">Rediger</Link>
                  {variant === "participants" && onPromote ? (
                    <button disabled={isBusy} onClick={() => onPromote(mid)} className="rounded-lg bg-red-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-60" title="Gjør til leder">Gjør til leder</button>
                  ) : null}
                  {variant === "leaders" && onDemote ? (
                    <button disabled={isBusy} onClick={() => onDemote(mid)} className="rounded-lg bg-white px-3 py-1.5 text-sm font-semibold text-neutral-900 ring-1 ring-neutral-300 hover:bg-neutral-100 disabled:opacity-60" title="Fjern som leder">Fjern</button>
                  ) : null}
                </div>
              </li>
            );
