// PATH: app/(protected)/members/[id]/page.tsx
"use client";
...
function calcTenure(start_date?: string | null, start_year?: number | null) {
  ...
  try {
    ...
    if (start_date) {
      ...
      since = fmtDateISO(start_date);
    } else if (start_year && Number.isFinite(start_year)) {
      start = new Date(Number(start_year), 0, 1);
      since = String(start_year);
    }
    if (start) {
      const now = new Date();
...
export default function MemberProfilePage() {
    ...
    (async () => {
      ...
      try {
        ...
        const { data: enr } = await supabase
          .from("enrollments")
          .select(
            "id, activity_id, role, activity:activities(id,name,type,archived)"
          )
  ...
  if (loading)
    return <main className="p-6 text-neutral-900">Laster medlem…</main>;
  if (!member)
    return (
      <main className="p-6 text-neutral-900">Fant ikke medlem.</main>
    );
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 text-neutral-900 space-y-6">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <a
                href={
                  hasEmail
                    ? `mailto:${member.email}?subject=Follies`
                    : undefined
                }
                onClick={(e) => {
              ...
              <button
                ...
                title={
                  member.archived ? "Allerede arkivert" : "Arkiver medlem"
                }
              >
                <IconArchive />
                {busyArchive
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          {/* Aktiviteter (aktive) */}
          <Card
            title={`Aktiviteter (aktive)`}
            badge={String(activeActs.length)}
          >
            {activeActs.length === 0 ? (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="font-medium text-neutral-900 truncate">
                        {e.activity?.name || "Ukjent aktivitet"}
                      </div>
          ...
          <Card
            title="Historikk – forestillinger"
            tone="muted"
            badge={String(pastShows.length)}
          >
            {pastShows.length === 0 ? (
              <p className="text-neutral-900">
                Ingen tidligere forestillinger registrert.
              </p>
            ) : (
              <ul className="space-y-3">
                {pastShows.map((e) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="font-medium text-neutral-900 truncate">
                        {e.activity?.name || "Ukjent forestilling"}
                      </div>
                      <div className="mt-0.5 text-xs text-neutral-700">
                        Rolle: {e.role === "leader" ? "Leder" : "Deltaker"} ·
                        Arkivert
                      </div>
                    </div>
                    {e.activity?.id ? (
...
function IconArchive() {
  return (
    <svg
      ...
      aria-hidden
    >
      <path d="M20.54 5.23L19.15 3.5A2 2 0 0017.57 3H6.43a2 2 0 00-1.58.5L3.46 5.23A2 2 0 003 6.57V19a2 2 0 002 2h14a2 2 0 002-2V6.57a2 2 0 00-.46-1.34zM6.24 5h11.52l.81 1H5.43l.81-1zM19 19H5V8h14v11z" />
    </svg>
  );
}
function IconClock() {
