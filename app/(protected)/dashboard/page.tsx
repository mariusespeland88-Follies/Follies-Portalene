...
function readCalendar(): AnyObj[] {
  const v1 = parseJSON<AnyObj[]>(readLS(CAL_V1), []);
  const fb = parseJSON<AnyObj[]>(readLS(CAL_FB), []);
  return [...v1, ...fb];
}
...
function leaderActivityIdsFromPerms(perms: AnyObj, myId: string): Set<string> {
  ...
  const arr: any[] = Array.isArray(perms?.entries)
    ? perms.entries
    : Array.isArray(perms)
    ? perms
    : [];
...
export default function DashboardPage() {
  ...
  const [myActivitiesStatus, setMyActivitiesStatus] = React.useState<
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const displayName =
        (me.member &&
          (fullName(me.member) ||
            `${me.member.first_name || ""} ${me.member.last_name || ""}`.trim())) ||
        "";
      ...
      if (alive) setMyActivitiesStatus("loading");
      try {
        ...
        if (res.ok) {
          ...
          if (alive && Array.isArray(j?.activities)) {
            ...
            setMyActivitiesStatus("ok");
          } else if (alive) {
            setMyActivitiesStatus("fail");
          }
        } else if (alive) {
          setMyActivitiesStatus("fail");
        }
      } catch {
        if (alive) setMyActivitiesStatus("fail");
      }
    })();
    return () => {
      alive = false;
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/dashboard/my-sessions?${qs.toString()}`);
        if (!res.ok) throw new Error(await res.text());
        const j = await res.json().catch(() => ({}));
        const list = Array.isArray(j?.sessions) ? (j.sessions as DbSession[]) : [];
  ...
  const upcomingFromLS = React.useMemo(() => {
    ...
    const items = readCalendar()
      ...
      .sort(
        (a, b) =>
          ((a as any)._start as Date).getTime() -
  ...
  const isMyActivitiesLoading =
    myActivitiesStatus === "loading" && myActivities.length === 0;
  const isIdentityConnecting =
    myActivitiesStatus === "idle" && !(me.id || "").trim() && !(me.email || "").trim();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <section className="rounded-xl border p-4 bg-white">
          ...
          {isMyActivitiesLoading ? (
            <div className="mt-3 text-gray-700">Laster aktiviteter…</div>
          ) : myActivities.length === 0 ? (
            <div className="mt-3 text-gray-700">
              {isIdentityConnecting ? "Kobler til medlemsprofil…" : "Ingen aktiviteter funnet."}
            </div>
          ) : (
