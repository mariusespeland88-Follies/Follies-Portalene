...
export function usePermissions() {
  ...
  async function refresh() {
    ...
    try {
      ...
      const memberByEmail = email
        ? membersLS.find((m) => emailOf(m)?.toLowerCase() === email!.toLowerCase())
        : null;
      ...
      if (supabase && sessionEmail) {
        ...
        if (!error && data && data.length > 0) {
          ...
          const r = Array.isArray(m.member_roles)
            ? m.member_roles.map((x: any) => String(x.role))
            : [];
          rolesFound = r;
  ...
  React.useEffect(() => {
    ...
    try {
      window.addEventListener("follies:auth-sync", onAuth);
    } catch {}
    try {
      window.addEventListener("follies:member-sync", onMember);
    } catch {}
    return () => {
      try {
        window.removeEventListener("follies:auth-sync", onAuth);
      } catch {}
      try {
        window.removeEventListener("follies:member-sync", onMember);
      } catch {}
    };
